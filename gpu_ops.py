...
class AdaptiveWarpAssignment:
    ...
    def _thread_per_node_attention(
        ...
        """
        Thread-per-node: fully batched dense matrix ops with degree-aware
        load balancing. Better when |A| is large enough to saturate GPU.

        Key difference from warp-per-node: sorts nodes by valid neighbor
        count (descending) so that high-degree nodes are processed first,
        overlapping their longer computation with the shorter tails.
        Uses standard cuBLAS batched GEMM for maximum throughput.
        """
        A, N, D_in = K_in.shape
        T = time_enc.size(-1)
        head_dim = Wq.size(0) // num_heads
        ...
        # Degree-aware sorting: process high-degree nodes first
        valid_counts = mask.sum(dim=1) if mask is not None else torch.full(
            (A,), N, device=K_in.device)
        degree_order = torch.argsort(valid_counts, descending=True)
        inv_order = torch.empty_like(degree_order)
        inv_order[degree_order] = torch.arange(A, device=K_in.device)

        # Reorder inputs by degree (high-degree first)
        Q_in = Q_in[degree_order]
        K_in = K_in[degree_order]
        time_enc = time_enc[degree_order]
        if mask is not None:
            mask = mask[degree_order]

        # Batched projections via cuBLAS GEMM
        Q = F.linear(Q_in, Wq)                         # [A, D_out]
        K_with_time = torch.cat([K_in, time_enc], -1)   # [A, N, D_in+T]
        K = F.linear(K_with_time, Wk)                   # [A, N, D_out]
        V_with_time = torch.cat([K_in, time_enc], -1)
        ...
        Q = Q.view(A, num_heads, head_dim)
        K = K.view(A, N, num_heads, head_dim).transpose(1, 2)
        V = V.view(A, N, num_heads, head_dim).transpose(1, 2)

        # Batched attention via torch.bmm (maps to cuBLAS)
        Q_flat = Q.reshape(A * num_heads, 1, head_dim)
        K_flat = K.reshape(A * num_heads, N, head_dim)
        V_flat = V.reshape(A * num_heads, N, head_dim)

        scores = torch.bmm(Q_flat, K_flat.transpose(1, 2))  # [A*H, 1, N]
        scores = scores.view(A, num_heads, N) / math.sqrt(head_dim)
        ...
        attn_flat = attn.reshape(A * num_heads, 1, N)
        out = torch.bmm(attn_flat, V_flat)  # [A*H, 1, d]
        out = out.view(A, D_out)
        out = F.linear(out, Wo)
